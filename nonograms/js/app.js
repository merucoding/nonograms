// alert('Привет, нереализовано до конца адаптив, смена темы игры. Сегодня закончу, прошу проверить позже)');

const body = document.body;
const sectionSide = createElement('section', 'sidebar', body);
const sideContainer = createElement('div', 'sidebar__container', sectionSide);

const settingBtn = createElement('button', 'sidebar__setting-btn btn', sideContainer);
...
const volumeBtn = createElement('button', 'sidebar__volume-btn btn', sideContainer);
volumeBtn.innerHTML = `
...
const sectionGame = createElement('section', 'game', body);
...
const rightSidebar = createElement('section', 'sidebar-right', body);
...
function gameInit() {
  temps[0].classList.add('select__option-active');
  ...
  localStorage.setItem('temp', JSON.stringify(data['5x5'][0]));
